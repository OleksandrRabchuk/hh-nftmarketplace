...
contract NftMarketplace {
    ...
    function listItem(address nftAddress, uint256 tokenId, uint256 price) external isOwner(nftAddress,tokenId, msg.sender) notListed(nftAddress,tokenId, msg.sender){
    ...
    function cancelItem(address nftAddress, uint256 tokenId) external isListed(nftAddress, tokenId) isOwner(nftAddress,tokenId, msg.sender){
        delete(s_listing[nftAddress][tokenId]);
    ...
    function withdrawProceeds() external {
        ...
        if(proceeds <= 0) revert NftMarketplace__NoFundsToWithdraw();
